...
public class DistanceTreeEvaluator {
    ...
    private final static String LOG= DistanceTreeEvaluator.class.getSimpleName();
	...
	public static double evaluate(Phylogeny tree, DistanceMatrix matrix) {
		...
		for (int row = 0; row < numSequences - 1; row++) {
			...
			for (int col = row + 1; col < numSequences; col++) {
				...
				if (commonParent != null) {
					...
					Log.i(LOG,nodeName1+" "+nodeName2+" Distance: "+distance+"Tree: "+treeDistance+" difference: "+Math.abs(distance - treeDistance));
				} else {
					Log.w(LOG,"Unable to find common parent with "+node1+" "+node2);
				}
			}
			path.clear();
		}
		...
		Log.i(LOG,"Average matrix distance: "+averageMatrixDistance);
		Log.i(LOG,"Average tree distance: "+averageTreeDistance);
		Log.i(LOG,"Average LS error: "+averageTreeErrorDistance);
